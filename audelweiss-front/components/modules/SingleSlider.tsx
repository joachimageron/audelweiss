...
export default function SingleSlider({ slides, className = '' }: SingleSliderProps) {
  ...
  return (
    <section className={`simple-slider relative ${className}`}>
      <button ref={prevArrow} className="absolute top-0 left-0 z-2 lg:flex hidden justify-center items-center h-full w-[12.5rem] opacity-0 cursor-pointer transition hover:opacity-100 focus:opacity-100" title='Revenir à la slide précédente'>
        ...
        {slides.map((slide, index) => {
          ...
          return (
            <SwiperSlide key={slide.id} className="relative flex justify-end items-center 2xl:px-[14rem] lg:px-[7rem] px-[4rem] lg:py-[5rem] py-[9rem] lg:min-h-[calc(100svh-11rem)]">
              <div className='simple-slide-image absolute top-0 left-0 w-full h-full'>
              ...
              <a href={slide.link.href} className="relative 2xl:pr-[10rem] lg:pr-[2rem] max-w-[68rem]" tabIndex={isActive ? 0 : -1} aria-hidden={!isActive}>
                <p className="simple-slide-subtitle relative mb-[2rem] pl-[5rem] text-[2rem] font-medium">{slide.subtitle}</p>
      ...
      <button ref={nextArrow} className="absolute top-0 right-0 z-2 lg:flex hidden justify-center items-center h-full w-[12.5rem] opacity-0 cursor-pointer transition hover:opacity-100 focus:opacity-100" title='Aller à la slide suivante'>
