          ...
          <div className="lg:order-2 order-0 flex flex-col justify-center h-full text-center flex-2">
            ...
            <ul className="flex justify-center gap-[2rem] flex-wrap">
              {footer.reseaux?.map((reseau, i) => (
                <li key={i} className="as--hover-filter-primary">
                  <CustomLink href={reseau.url} className="block p-[10px] border border-white rounded-full as--filter-white transition hover:border-primary">
                    {reseau.icon?.url && (
                      <Image
